...
bool split(const mat<rgb> & img, const std::string & filename){
	
	mat<uint8_t> R(split_dim(img, 0));
	mat<uint8_t> G(split_dim(img, 1));
	mat<uint8_t> B(split_dim(img, 2));
	//std::string partial_filename = filename.substr(filename.find_last_of(".pam"));
	
	save_pam(R, filename + "_R.pam");
	save_pam(G, filename + "_G.pam");
	save_pam(B, filename + "_B.pam");

	return true;
}


bool combine(const std::string & filename){
	...
	if(!load_pam(R, filename + "_R.pam"))
		return false;
	load_pam(G, filename + "_G.pam");
	load_pam(B, filename + "_B.pam");

	auto rows = R.rows();
	auto cols = G.cols();
	mat<rgb> RGB(rows, cols);
	for(int r = 0; r<rows; r++){
		for(int c = 0; c<cols; c++){
	...
	save_pam(RGB, filename + "_reconstructed.pam");
	return true;
}
...
int main(int argc, char* argv[]) {
	if (argc != 2) {
		std::cout << "Wrong syntax, correct usage should be 'combine <filename>";
		return EXIT_FAILURE;
	}
